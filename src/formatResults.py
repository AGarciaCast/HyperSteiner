...
MODES_SCALABILITY = ["3+Simple", "3+Precise", "4+Simple", "4+Precise"]
...
def rearrange_table_convergence(mydf):
    auxDf = None
    for mode in MODES_CONVERGENCE:
        extractedDF = mydf[mydf[("mode")] == mode]
...
def process_df_convergence(path, title="Conv2Ideal"):

    raw_df = read_df(path=path, title=title)
    raw_df = raw_df.drop(columns=COLUMNS_TO_DROP + ["avgTime", "avgFST3", "avgFST4"])
    raw_df["perImprov"] = raw_df["perImprov"] * 100

    full_df = rearrange_table_convergence(raw_df)
